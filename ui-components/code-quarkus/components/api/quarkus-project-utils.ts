...
function syncParamsInQuery(filterParam: string = '', project: QuarkusProject | undefined): void {
  ...
  window.history.replaceState(null, '', `/${generateParamQuery(formatParam(FILTER_PARAM_NAME, filterParam), generateProjectQuery(project, false, false))}`);
}
...
export function normalizeStreamKey(recommendedPlatformKey: String, streamKey?: String) {
  if (streamKey == null) {
    return null;
  }
  return streamKey!.indexOf(":") >= 0 ? streamKey! : `${recommendedPlatformKey}:${streamKey}`;
}

export function getQueryStreamKey(): String | undefined {
  const p = getQueryParams();
  return p && p['S'] && p['S'].toString();
}
